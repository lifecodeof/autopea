...
pageTest("PhotopeaMutexes - should return same instance for same page", async ({ page }) => {
  const mutexes1 = PhotopeaMutexes.of(page.page)
  const mutexes2 = PhotopeaMutexes.of(page.page)
  expect(mutexes1).toBe(mutexes2)
})

pageTest("PhotopeaMutexes - should handle concurrent access", async ({ page }) => {
  const mutexes = PhotopeaMutexes.of(page.page)
...
pageTest("PhotopeaMutexes - should allow parallel access to different mutexes", async ({ page }) => {
  const mutexes = PhotopeaMutexes.of(page.page)
